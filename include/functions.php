...
function checkname ($username){   // Vartotojo vardo sintakse
 ...
 function checkdb($username) {  // iesko DB pagal varda, grazina {vardas,slaptazodis,lygis,id} ir nustato name_error
		 ...
		 $sql = "SELECT * FROM " . TBL_USERS. " WHERE prisijungimo_vardas = '$username'";
	 	 echo $sql;
	  	 ...
	  	 {  // neradom vartotojo DB
		    $_SESSION['name_error']=
			 "<font size=\"2\" color=\"#ff0000\">* Tokio vartotojo nėra</font>";
		   	...
		   	$name=$row["vardas"];
			$surname=$row["pavarde"];
			$date=$row["gimimo_data"];
           	$uname= $row["prisijungimo_vardas"];
			$upass= $row["slaptazodis"]; 
			$umail = $row["el_pastas"];
			$uid= $row["userid"];
           	$ulevel=$row["vartotojo_tipas"]; 
		 }
         return array($name,$surname,$date,$uname,$upass,$umail,$uid,$ulevel);
 }
